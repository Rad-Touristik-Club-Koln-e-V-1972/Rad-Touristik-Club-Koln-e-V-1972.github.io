              ...
              <q-card>
                ...
                <q-card-section>
                  <q-markup-table flat>
                    <thead>
                      <tr>
                        <th class="text-left" scope="col">Beitrags-/Mitgliedsform</th>
                        <th class="text-right" scope="col">Jahresbeitrag</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="it in feeGroupsRepo.getByCategory(EFeeGroup.MembershipRegistration)?.fees" :key="it.id">
                        <td v-html="it.name" />
        ...
        <q-card-section>
          <q-stepper v-model="step" animated vertical>
            ...
            <q-step :done="step > 4" :name="4" title="Mitgliedsbeitrag">
              <q-stepper-navigation>
                <c-membership-fee v-model="value.fee" />
                <q-btn color="primary" label="Weiter" @click="step += 1" />
                <q-btn class="q-ml-sm" color="primary" flat label="Zurück" @click="step -= 1" />
              </q-stepper-navigation>
            </q-step>
            <q-step :done="step > 5" :name="5" title="BDR-Mitgliedschaft">
              <q-stepper-navigation>
                <c-bdr-membership v-model="value.bdr" />
                <q-btn color="primary" label="Weiter" @click="step += 1" />
                <q-btn class="q-ml-sm" color="primary" flat label="Zurück" @click="step -= 1" />
              </q-stepper-navigation>
            </q-step>
            ...
            <q-step :done="step > 7" :name="7" title="Einwilligungserklärung Datenverarbeitung">
              <q-stepper-navigation>
                <c-consent-data-usage v-model="value.consentPrivacyPolicySignature" />
...
import FeeGroupRepository from 'stores/FeeGroupRepository'

const feeGroupsRepo = useRepo(FeeGroupRepository)

const images = useRepo(GalleryAlbumEntry).make([
  {
    imageUrl: new URL('https://content.rtc-koeln.de/pages/membership-registration/Din-A4-Flyer-Vorteile-Verein-RGB.pdf'),
    previewUrl: new URL('https://content.rtc-koeln.de/pages/membership-registration/Din-A4-Flyer-Vorteile-Verein-RGB.avif'),
  },
])
const step = ref(1)
const tab = ref('infos')
const value = useRepo(MembershipRegistration).make()
</script>
