<template>
  <q-card flat>
    <q-toolbar>
      <q-btn v-if="mode !== ECalendar.List" :icon="mdiChevronLeft" fab flat @click="calendarRef!.prev()" />
      <q-btn v-if="mode !== ECalendar.List" outline @click="calendarRef!.moveToToday()">Heute</q-btn>
      <q-btn v-if="mode !== ECalendar.List" :icon="mdiChevronRight" fab flat @click="calendarRef!.next()" />
      <q-toolbar-title v-if="mode !== ECalendar.List && !$q.screen.xs">{{ title }}</q-toolbar-title>
      <q-space />
      <q-select v-model="mode" :options="Object.values(ECalendar)" filled label="Darstellung" style="min-width: 161px">
        <template #prepend>
