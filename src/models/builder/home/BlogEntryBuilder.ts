...
import BlogEntry from "@/models/entities/home/BlogEntry";
...
export default class BlogEntryBuilder extends ABuilder {
  ...
  setAlbumIDs = (value: Record<string, string> | string[] | string): this => {
    if (Array.isArray(value))
      this.blogEntry.albumIDs = value.reduce<Record<string, string>>(
        (map, it) => {
          map[it] = "";
          return map;
        },
      ...
      this.blogEntry.albumIDs = { [value]: "" };
    else
      this.blogEntry.albumIDs = Object.assign(
        this.blogEntry.albumIDs,
