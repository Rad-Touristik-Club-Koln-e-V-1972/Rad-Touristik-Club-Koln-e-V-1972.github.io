...
export default () => ({
  format: (start: Date, end: Date | null = null, allDay = true) => {
    let value: string
    end?.toString()
  ...
  formatTime: (start: Date, end: Date | null = null) => (end ? timeFormatter.formatRange(start, end) : `${date.formatDate(start, 'H:mm')} Uhr`),
