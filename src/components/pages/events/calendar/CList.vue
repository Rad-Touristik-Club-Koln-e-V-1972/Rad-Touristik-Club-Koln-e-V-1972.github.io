<template>
  <q-card>
    ...
    <q-table
      ...
      :filter-method
      :pagination
      :rows="useCalendarStore().all"
      :table-row-class-fn="getClass"
      :table-row-style-fn="getStyle"
      separator="horizontal"
    >
      <template #body-cell-name="props">
        <q-td :props="props">
          <a
            :class="getClass(props.row)"
            :href="props.row.url?.toString()"
            target="_blank"
            >{{ props.row.name }}</a
          >
        </q-td>
...
import type Filter from "@/models/entities/events/calendar/Filter";
...
const columns: QTableColumn<Event>[] = [
  ...
  },
  {
    ...
    label: "Termin",
    name: "datetime",
    sort: (a: string, b: string, rowA: Event, rowB: Event) =>
      sortDateTime(rowA, rowB),
    sortable: true,
    sortOrder: "ad"
  },
  {
...
const filter = ref<Filter>({
  ...
  dateRange: {
    from: `01.01.${dateTime.today.value.getFullYear().toFixed()}`,
    to: `31.12.${(dateTime.today.value.getFullYear() + 1).toFixed()}`
  },
  search: ""
});
...
const filterMethod = (rows: readonly Event[], terms: Filter): Event[] => {
  ...
  if (terms.categories.length)
    tmp = tmp.filter(item => terms.categories.includes(item.category));
  ...
  if (terms.dateRange) {
    ...
    if (is.object(terms.dateRange)) {
      ...
      if (terms.dateRange.to)
        to = date.extractDate(terms.dateRange.to, "DD.MM.YYYY");
  ...
  if (terms.search)
    tmp = tmp.filter(item =>
      (item.kilometer.toFixed() + item.contact + item.name)
        .toLowerCase()
        .includes(terms.search.toLowerCase())
    );
...
const getStyle = (row: Event) => {
  ...
  if (row.category === EEvent.Abgesagt)
    style += "text-decoration: double line-through ";
