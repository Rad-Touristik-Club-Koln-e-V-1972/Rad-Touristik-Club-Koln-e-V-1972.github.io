<template>
    <v-card-text id="CConsentDataUsage">
        ...
        <v-row>
            <v-col>
                <v-text-field v-model="value.location" dense label="Ort" @input="emitUpdate" />
            </v-col>
            <v-col>
                <c-date-picker v-model="value.date" dense label="Datum" @input="emitUpdate" />
            </v-col>
        </v-row>
        <v-row>
            <v-col>
                <c-signature-form v-model="value.signature" label="Unterschrift (Bei Minderjährigen: Unterschrift des Erziehungsberechtigten)" @input="emitUpdate" />
            </v-col>
        </v-row>
    </v-card-text>
</template>
...
const props = defineProps<{ value: Signature }>()

const value = ref(props.value ?? new Signature())
