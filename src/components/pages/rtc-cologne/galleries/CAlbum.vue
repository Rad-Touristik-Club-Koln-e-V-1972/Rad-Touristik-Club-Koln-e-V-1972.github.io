<template>
  <q-card-section>
    <q-tabs
      v-show="
        Object.keys(modelValue.images).length &&
        modelValue.youtubeVideoIds.length
      "
      v-model="tab"
    ...
    <q-tab-panels v-model="tab">
      <q-tab-panel name="Bilder">
        <q-tabs
          v-show="Object.keys(modelValue.images).length > 1"
          v-model="tabPictures"
          active-bg-color="primary"
          active-color="accent"
          indicator-color="secondary"
        >
          <q-tab
            v-for="name in Object.keys(modelValue.images)"
            :key="name"
            :label="name"
            :name
          />
        ...
        <q-tab-panels v-model="tabPictures">
          <q-tab-panel
            v-for="(item, key) in modelValue.images"
            :key
            :name="key"
          >
      ...
      <q-tab-panel name="Videos">
        <!-- TODO Workaround until the browsers support native masonry walls. See https://caniuse.com/?search=masonry
                    ...
                    & https://github.com/w3c/csswg-drafts/issues?q=is%3Aopen+label%3Acss-grid-3+masonry -->
        <masonry-wall
          :column-width="useQuasar().platform.is.mobile ? 280 : 560"
          :gap="16"
          :items="modelValue.youtubeVideoIds"
        >
...
const tabPictures = ref(
  props.album ? props.album : Object.keys(props.modelValue.images)[0]
);
