<template>
  <div>
    <q-card
      ...
      flat
      @click="eventRef!.showEvent(nextEvents[0]!)"
    >
...
const calcCountdown = (nextEvent: Event) => {
  ...
  hours.value = Math.floor(
    (milliseconds % _millisecondsDay) / _millisecondsHour
  );
  minutes.value = Math.floor(
    (milliseconds % _millisecondsHour) / _millisecondsMinute
  );
};
const getPercentage = (max: number, value: number) =>
  Math.round((100 / max) * value);
