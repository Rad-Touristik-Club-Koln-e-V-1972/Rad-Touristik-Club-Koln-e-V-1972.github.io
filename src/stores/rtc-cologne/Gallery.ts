...
import _2026 from "./gallery/2026/Gallery";

export default defineStore("gallery", () => {
  const sortByDate: (galleries: Gallery[]) => Gallery[] = (
    galleries: Gallery[]
  ) => galleries.toSorted((a, b) => b.start.getTime() - a.start.getTime());

  const groupedByYear: ComputedRef<Record<string, Gallery[]>> = computed(
    () => ({
      2026: sortByDate(_2026),
  ...
  return {
    ...
    findByIds: (...ids: string[]) =>
      all.value.filter(gallery => ids.includes(gallery.id)),
    groupedByYear
  };
});
